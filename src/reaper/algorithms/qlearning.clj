(ns reaper.algorithms.qlearning
  "An implementation of backwards TD(lambda) from Sutton and Barto's
        ...
        [[qnext enext]
         (loop
           [s initial-state
            ...
            q q 
            e e]
           (let [s' (m s (policy fe q tk sp s))
                 a (last (second s'))
                 a' (policy q tk sp s')
                 a*-score (apply max (map #(get q [(fe s') %] 0)) (sp s'))
                 a* (if #(= a*-score (get q [(fe s) a'] 0)) a' (util/argmax #(get q [s %]) (sp s)))
                 delta (- (+ (reward s') (* y (get q [(fe s') a*] 0))) (get q [(fe s) a] 0))
                 e (assoc e [(fe s) a] (inc (get e [(fe s) a] 0)))
                 q (if (= nil (q [(fe s) a])) (assoc q [(fe s) a] 0) q)
                 q (util/fkmap #(+ %2 (* alphak delta (e %1 0))) q)
                 e (if (= a' a*)
                     (assoc e [(fe s) a] (* y lambda (get e [(fe s) a])))
                     (assoc e [(fe s) a] 0))]
